...
5. 模型幻觉（Hallucination）<sup>[11]</sup>是大语言模型当前存在的关键局限性之一。本章介绍了缓解幻觉的方法（如检索增强生成、多步推理、外部工具调用）
   ...
   - 答：1. RAG的工作原理是外挂一个知识库（文档知识库），在将用户的输入喂给大模型之前先从知识库中检索出对应的信息，将其整合到上下文中一起送给大模型，使大模型生成基于事实的回答。适用场景主要是专业知识场景，比如金融或者法律相关的智能体。2.多步推理使让大模型在面对复杂任务时一步一步地输出其推理过程，使其逻辑更严谨。适用场景主要是需要严密的逻辑的场景，比如解数学题3，外部工具调用的原理是给大模型提供一些外部工具（比如联网搜索，运行脚本），使其可以实时地获取信息来对自己的答案进行验证。外部工具调用适用于日常对话，比如询问大模型一些常识或者不是很严谨的知识。
...
6. 假设你要设计一个论文辅助阅读智能体，它能够帮助研究人员快速阅读并理解学术论文，包括：总结论文研究的核心内容、回答关于论文的问题、提取关键信息、比较多篇不同论文的观点等。请回答：
   ...
   - 答：我会选择GPT作为模型的底座。需要考虑模型的成本，响应速度，幻觉率等等
   - 如何设计提示词来引导模型更好地理解学术论文？学术论文通常很长，可能超过模型的上下文窗口限制，你会如何解决这个问题？
   - 答：在提示词中可以给大模型设定对应的角色，比如“你是一名xxx领域的研究专家”。对于长学术论文，可以对论文进行适当的压缩，使其在保留重要信息的同时减少上下文窗口的占用。
   - 学术研究是严谨的，这意味着我们需要确保智能体生成的信息是准确客观忠于原文的。你认为系统中加入哪些设计能够更好的实现这一需求？
   - 答：可以在提示词中强制命令大模型不要输出在原文中不存在的内容，以及大模型在输出信息时指出这条信息来自原文的哪些部分，并且可以将大模型输出的内容送给另一个大模型来检查输出是否忠于原文。
