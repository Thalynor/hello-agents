...
1. 本章介绍了三种经典的智能体范式:`ReAct`、`Plan-and-Solve` 和 `Reflection`。请分析:
   ...
   - 答：1.ReAct范式将行动与思考融合到每一轮循环中，在每一轮循环中都先思考后行动。2.Plan-and-Solve范式则是将思考与行动分离，先思考再行动。3.Reflection范式则是先行动，在通过思考进行反思，然后根据反思再修改自己的行动。
   - 如果要设计一个"智能家居控制助手"（需要控制灯光、空调、窗帘等多个设备，并根据用户习惯自动调节），你会选择哪种范式作为基础架构？为什么？
   - 答：我会使用ReAct范式，因为控制家具是一个对实时性要求很高的场景，而且任务简单，不需要复杂的逻辑推理。
...
2. 在4.2节的 `ReAct` 实现中，我们使用了正则表达式来解析大语言模型的输出（如 `Thought` 和 `Action`）。请思考:
   ...
   - 答：`(.*)` 是**贪婪匹配**，如果 Action 中有嵌套括号（如 `Search[英伟达RTX 5090[旗舰版]]`），会匹配到最后一个 `]`，导致解析结果错误。

   - 除了正则表达式，还有哪些更鲁棒的输出解析方案？
   - 答：可以通过结构化输出，比如让大模型以JSON格式输出答案，然后通过JSON库解析大模型的输出
...
6. 提示词工程是影响智能体最终效果的关键技术。本章展示了多个精心设计的提示词模板。请分析:
   ...
   - 答：ReAct的提示词要求大模型一次输出思考和行动，这使得大模型可以直接完成一轮”思考-行动“的循环。Plan-and-Solve的提示词分为两部分，一个是规划agent的提示词，一个是执行agent的提示词，这使得规划和行动可以分离开来。
   - 在4.4.3节的 `Reflection` 提示词中，我们使用了"你是一位极其严格的代码评审专家"这样的角色设定。尝试修改这个角色设定（如改为"你是一位注重代码可读性的开源项目维护者"），观察输出结果的变化，并总结角色设定对智能体行为的影响。
   - 答：角色设定会影响agent的主要关注点，改变角色设定会使agent更专注的自身角色的职责。
